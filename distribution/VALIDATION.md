...
- Ten English marketing screenshots and twelve raw captures remain. The retained English sets were visually reviewed; French assets have been removed at the owner's request.
...
- The English-only source commit `80efb57` passed GitHub core, persistence, audio, competition, scenery, asset and simulator-build CI checks.
- The owner connected Xcode. **App Store export succeeded**, resolving the earlier No Accounts/distribution-profile error. Build 17 was uploaded, processed by Apple, and selected for version 1.0.0.

## Evidence and limits

Local evidence is kept in `artifacts/app-store-release/evidence/`, excluded from public Git. The private Apple contact is excluded from public Git and public HTML. The three English pages return HTTP 200 and match the current source byte-for-byte; all three old French pages return HTTP 404.
