...
final class StuntTests: XCTestCase {
    ...
    #if DEBUG
    func testBackflipUIFixtureEmitsOneRealSafeReceptionInBothModes() {
        for mode: RunMode in [.weekly, .endless] {
            let simulation = GameSimulation.backflipFixtureForTesting(mode: mode)
            ...
            XCTAssertEqual(creditedBackflips, 1)
            XCTAssertEqual(creditedFrontflips, 0)
        }
    }
    #endif
