...
All **40 achievements / 1,000 points** are available locally. Their Game Center synchronization is **deferred and disabled for this candidate** (`CrocoGameCenterAchievementsEnabled` absent/false).

The owner authorized remote creation, then **stopped further entry on September 24, 2026**. Seven complete achievements were saved: `stunt.backflip.first`, `stunt.frontflip.first`, `stunt.double.landed`, `stunt.triple.landed`, `stunt.total.10`, `endless.distance.1000` and `weekly.finish.1`. No partially completed entry remains. Preserve these seven without more creation, edits or deletion; none was submitted for review by this achievement-entry work. The remaining 33 remote entries are deferred. This does not alter the complete local catalog or unlock progression.
...
All **40 distinct achievement badges are generated and visually reviewed locally** in [distribution/game-center/achievements](../distribution/game-center/achievements), with an image manifest and contact sheet. The images are opaque 1024 × 1024 sRGB PNGs at 72 ppi. Seven saved remote achievements have their artwork/localizations; the remaining remote entries and their uploads are deferred. Version association and review/activation are not established by this guide. SF Symbols in the local panel are separate from these upload assets. For App Store Connect, supply 1024 × 1024 RGB PNG/JPEG/JPG images at a minimum of 72 ppi, plus at least one localization containing display name, pre-earned description and earned description. [Required metadata and image format](https://developer.apple.com/help/app-store-connect/reference/game-center/achievements).
...
Only if the owner resumes the deferred remote achievement work, configure the reviewed IDs, points, localization and artwork in App Store Connect, associate them with the app version, and perform approved sandbox testing. Only then enable the Boolean `CrocoGameCenterAchievementsEnabled` in the app's Info.plist. The key is currently absent, which means disabled; the partial remote catalog must not be enabled. Debug and Release use the same activation policy and the same local achievement ledger. The standard `-ui-testing` launch flag keeps automated UI tests offline regardless of that setting. No remote reset API is used.
