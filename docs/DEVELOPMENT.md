...
Validate the imported MP3s without speaker playback with `scripts/check-death-sounds.swift` (compile alongside `AudioService.swift`). The existing `-ui-testing` mode accepts `-death-sound-index 2` for a repeatable long-clip UI check; normal launches always select randomly. Source hashes are recorded in [death-sounds.json](death-sounds.json).
