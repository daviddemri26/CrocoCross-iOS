...
// Run from the repository root: swift scripts/check-rocco-assets.swift
// Reads the exact manifest used by RoccoArtwork; never rewrites any source image.
...
struct Manifest: Decodable { let schemaVersion: Int, rig: Rig, parts: [Part] }

func check(_ condition: @autoclosure () -> Bool, _ message: String) throws {
    if !condition() { throw NSError(domain: "RoccoAssetCheck", code: 1, userInfo: [NSLocalizedDescriptionKey: message]) }
}

do {
    let root = URL(fileURLWithPath: CommandLine.arguments.dropFirst().first ?? FileManager.default.currentDirectoryPath)
    let directory = root.appendingPathComponent("App/Resources/GameAssets/RoccoRig", isDirectory: true)
    let manifest = try JSONDecoder().decode(Manifest.self, from: Data(contentsOf: directory.appendingPathComponent("manifest.json")))
    try check(manifest.schemaVersion == 1, "Unsupported Rocco manifest version")
    let required: Set<String> = ["bike", "torso", "pelvis", "upper-arm", "forearm", "thigh", "calf", "boot", "fork", "swingarm", "wheel"]
    try check(Set(manifest.parts.map(\.part)).isSuperset(of: required), "Missing required Rocco parts")
    try check(Set(manifest.parts.map(\.part)).count == manifest.parts.count, "Duplicate part identifiers")
    var report: [[String: Any]] = []
    for part in manifest.parts {
        try check(part.assetName == "rocco-" + part.part, "Unexpected source path for \(part.part)")
        try check(part.landmarks["pivot"] != nil, "Missing \(part.part) pivot")
        ...
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil), let image = CGImageSourceCreateImageAtIndex(source, 0, nil) else {
            throw NSError(domain: "RoccoAssetCheck", code: 2, userInfo: [NSLocalizedDescriptionKey: "Cannot read \(url.path)"])
        }
    ...
    let ankle = bike.local("footpeg") + Point(x: -sole.x, y: -sole.y)
    for translation in stride(from: -manifest.rig.pelvisTravel, through: manifest.rig.pelvisTravel + 0.00001, by: 0.01) {
        for angle in stride(from: -manifest.rig.torsoAngleTravel, through: manifest.rig.torsoAngleTravel + 0.00001, by: 0.025) {
            ...
            let armDistance = shoulder.distance(to: bike.local("grip"))
            let legDistance = hip.distance(to: ankle)
            try check(armDistance < armA + armB && armDistance > abs(armA - armB), "Unreachable handlebar within rider posture envelope")
            try check(legDistance < legA + legB && legDistance > abs(legA - legB), "Unreachable footpeg within rider posture envelope")
            armReach = max(armReach, armDistance); legReach = max(legReach, legDistance)
            testedPoses += 1
        }
    }
    let result: [String: Any] = ["status": "passed", "assets": report, "postureSamples": testedPoses,
                                "maximumArmReach": armReach, "maximumLegReach": legReach,
                                "armLength": armA + armB, "legLength": legA + legB]
    print(String(data: try JSONSerialization.data(withJSONObject: result, options: [.prettyPrinted, .sortedKeys]), encoding: .utf8)!)
} catch {
    FileHandle.standardError.write(Data(("Rocco asset check failed: \(error.localizedDescription)\n").utf8))
    exit(1)
}
