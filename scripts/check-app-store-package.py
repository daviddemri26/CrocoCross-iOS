...
def require(ok,label):
 ...
 checks.append(label)
def submission_requirements(info,readiness):
 """Validate explicit release intent without treating deferred remote work as done."""
 labels=[]
 def check(ok,label):
  if not ok:raise ValueError(label)
  labels.append(label)
 mode=readiness.get('achievementReleaseMode')
 check(mode in ('local-only','game-center'),'achievement release mode is explicitly local-only or game-center')
 enabled=info.get('CrocoGameCenterAchievementsEnabled',False)
 check(isinstance(enabled,bool),'achievement reporting Info.plist flag is Boolean or absent')
 check(readiness.get('remoteAchievementSyncEnabled') is enabled,'readiness achievement sync flag matches Info.plist')
 check(enabled == (mode=='game-center'),'achievement release mode agrees with the actual reporting flag')
 gates=readiness.get('submissionGates')
 check(isinstance(gates,dict),'submission gates are explicit')
 achievement_gates={'fortyAchievementsConfiguredAndAssociated','sandboxAchievementRoundTripPassed'}
 required_gates=achievement_gates|{'appleVersionAndBuildVerified','fourLeaderboardsConfiguredAndAssociated',
  'gameCenterArtworkComplete','releaseScreenshotsReviewed','sandboxGameCenterRoundTripPassed'}
 check(required_gates.issubset(gates),'all required submission gates are present')
 for gate,passed in sorted(gates.items()):
  check(isinstance(passed,bool),'submission gate is Boolean: '+gate)
  if not enabled and gate in achievement_gates:
   # False remains an honest record of deferred remote achievement work.
   labels.append('remote achievement gate not required in local-only mode: '+gate)
  else:
   check(passed is True,'submission gate: '+gate)
 return labels
...
if '--submission' in sys.argv:
 readiness=json.loads((D/'release-readiness.json').read_text())
 try:checks.extend(submission_requirements(info,readiness))
 except ValueError as error:raise SystemExit('FAIL: '+str(error))
 print('Submission prerequisites explicitly confirmed ('+readiness['achievementReleaseMode']+' achievements)')
